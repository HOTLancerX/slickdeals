...
const MenusUser: React.FC<MenusProps> = ({ items }) => {
  ...
  return (
    <div className='flex flex-row overflow-x-auto no-ber w-full bg-white'>
      {items.map((item) => (
        <Link
          key={item.link}
          href={item.link}
          className={`flex-none px-3 py-2 border-b-[3px] hover:border-secondary ${
            pathname === item.link ? 'border-main text-main' : 'border-white text-gray-700 dark:border-dark'
          }`}
        >
